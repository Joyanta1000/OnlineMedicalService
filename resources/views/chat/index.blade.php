...
-->
    ...
    <div id="frame">
        <div id="sidepanel">
            <div id="profile">
                <div class="wrap">
                    <img id="profile-img" src="{{ url($ProPic) }}" class="online" alt="" />

                    <p>{{ $Name }}</p>
                    <i class="fa fa-chevron-down expand-button" aria-hidden="true"></i>
                    <div id="status-options">
                        <ul>
                            @if (Cache::has('user-is-online-' . session()->get('id')))
                    ...
                    <div id="expanded">
                        <a type="button" class="btn btn-primary" style="text-decoration: none; color: white;"
                            href="{{ route(session()->get('role') == 2 ? 'doctor_dashboard' : 'patient_dashboard') }}"><i
                                class="fa fa-dashboard"></i> Back to dashboard </a>
                    </div>
                </div>
            </div>
            <div id="search">
                ...
                success: function(dataResult) {
                    ...
                    $.each(resultData, function(index, row) {
                        ...
                        if (row.senders_id == session_id || row.recievers_id == session_id) {
                            if (row.is_seen == 0) {
                                bodyData +=
                                    "<div style=' color: white; font-weight: bold; tex-decoration:none;' data-visualcompletion='ignore-dynamic'><a onclick='myFunction(" +
                                    row.id +
                                    ")'><li class='contact'><div class='wrap'><span class='contact-status'></span>";

                                if (role == 2) {
                                    bodyData += "<img style='height: 45px;' src='" + row
                                        .patients_profile_picture +
                                        "' alt='' /><div class='meta'><p class='name'>" + row
                                        .patients_first_name + "</p>";
                                } else if (role == 3) {
                                    bodyData += "<img style='height: 45px;' src='" + row
                                        .profile_picture +
                                        "' alt='' /><div class='meta'><p class='name'>" + row
                                        .doctors_first_name + "</p>";
                                }
                                if (row.message != null) {
                                    ...
                                    if (row.senders_id == session_id) {
                                        u = "<caption>you: </caption>";
                                    }
                                    bodyData += "<p class='preview'>"+(u ? u : '')+"Sent a file</p>";
                                }
                                bodyData += "</p></div>";
                                bodyData += "</div></li></a></div>";
                            } else {
                                bodyData +=
                                    "<div style=' color: gray; tex-decoration:none;' data-visualcompletion='ignore-dynamic'><a onclick='myFunction(" +
                                    row.id +
                                    ")'><li class='contact'><div class='wrap'><span class='contact-status '></span>";
                                if (role == 2) {
                                    bodyData += "<img style='height: 45px;' src='" + row
            ...
            $.ajax({
                ...
                success: function(dataResult) {
                    ...
                    $.each(resultData, function(index, row) {
                        ...
                        console.log((minutes < 60 ? (minutes < 1 ? seconds + " seconds ago" : minutes +
                            " minutes ago") : hours + " hours ago"));
                        ...
                        if (row.senders_id != session_id) {
                            ...
                            if (dataResult.SendersProfilePicture.title == row.senders_id) {
                                if (dataResult.SendersProfilePicture.searchable.profile_picture !=
                                    null) {
                                    messageData +=
                                        "<li class='replies'><img style='height: 32px;' src='" +
                                        dataResult.SendersProfilePicture.searchable.profile_picture +
                                        "' alt='' />";
                                } else if (dataResult.SendersProfilePicture.searchable
                                    .patients_profile_picture != null) {
                                    messageData +=
                                        "<li class='replies'><img style='height: 32px;' src='" +
                                        dataResult.SendersProfilePicture.searchable
                                        .patients_profile_picture + "' alt='' />";
                                }
                            } else if (dataResult.RecieversProfilePicture.title == row.senders_id) {
                                if (dataResult.RecieversProfilePicture.searchable.profile_picture !=
                                    null) {
                                    messageData +=
                                        "<li class='replies'><img style='height: 32px;' src='" +
                                        dataResult.RecieversProfilePicture.searchable.profile_picture +
                                        "' alt='' />";
                                } else if (dataResult.RecieversProfilePicture.searchable
                                    .patients_profile_picture != null) {
                                    messageData +=
                                        "<li class='replies'><img style='height: 32px;' src='" +
                                        dataResult.RecieversProfilePicture.searchable
                                        .patients_profile_picture + "' alt='' />";
                                }
                            }
                            ...
                            // }
                            if (row.message != null) {
                                messageData += "<p>" + row.message + "<br><small>" + (minutes < 61 ? (
                                    minutes < 1 ? seconds + " seconds ago" : minutes +
                                    " minutes ago") : hours + " hours ago") + "</small></p>";
                            }
                            ...
                            if (row.file != null) {
                                ...
                                if (fileName.split('.').pop() != 'jpg' && fileName.split('.').pop() !=
                                    'jpeg' && fileName.split('.').pop() != 'JPG' && fileName.split('.')
                                    .pop() != 'JPEG' && fileName.split('.').pop() != 'png' && fileName
                                    .split('.').pop() != 'PNG') {
                                    messageData +=
                                        "<br><p style='margin-left: 20px;'><a style='height: 100px; width: 100px; border-radius: 2px;' href='" +
                                        row.file +
                                        "' alt=''><i class='fa fa-file-o' style='font-size:48px;color:white'></i></a><br><small>" +
                                        (minutes < 61 ? (minutes < 1 ? seconds + " seconds ago" :
                                            minutes + " minutes ago") : hours + " hours ago") +
                                        "</small></p>";
                                } else {
                                    messageData += "<br><p style='margin-left: 20px;'><a href='" + row
                                        .file +
                                        "'><img style='height: 100px; width: 100px; border-radius: 2px;' src='" +
                                        row.file + "' alt='' /></a><br><small>" + (minutes < 61 ? (
                                            minutes < 1 ? seconds + " seconds ago" : minutes +
                                            " minutes ago") : hours + " hours ago") + "</small></p>";
                                }
                            }
                            ...
                            if (dataResult.SendersProfilePicture.title == row.senders_id) {
                                if (dataResult.SendersProfilePicture.searchable.profile_picture !=
                                    null) {
                                    messageData += "<li class='sent'><img style='height: 32px;' src='" +
                                        dataResult.SendersProfilePicture.searchable.profile_picture +
                                        "' alt='' />";
                                } else if (dataResult.SendersProfilePicture.searchable
                                    .patients_profile_picture != null) {
                    ...
                    messageData += "</ul></div>";
                    messageData +=
                        "<div class='message-input'><div class='wrap'><input type='text' name='message' id='message' placeholder='Write your message...' /><label for='fileInput'><i class='fa fa-paperclip attachment' aria-hidden='true'></i></label><input type='file' name='file' class='fileInput hide' id='fileInput'><button class='submit' onclick='submitMessage(" +
                        recievers_id + "," + senders_id + "," + message_id +
                        ")'><i class='fa fa-paper-plane' aria-hidden='true'></i></button></div></div>";
                    ...
                    $.each(resultData, function(index, row) {
                        ...
                        if (row.is_seen == 0) {
                            ...
                            if (role == 2) {
                                ...
                                bodyData += "<img style='height: 45px;' src='" + row.profile_picture +
                                    "' alt='' /><div class='meta'><p class='name'>" + row
                                    .doctors_first_name + "</p>";
                            }
                            if (row.message != null) {
                                ...
                                bodyData += row.message +
                                    "&nbsp;<small style='color:white; right: 20px; margin-left:20px;'>" +
                                    (minutes < 61 ? (minutes < 1 ? seconds + " seconds ago" : minutes +
                                        " minutes ago") : hours + " hours ago") + "</small></p>";
                            } else {
                                bodyData += "<p class='preview'>";
                                if (row.senders_id == session_id) {
                                ...
                                bodyData +=
                                    "Sent a file&nbsp;<small style='color:white; right: 20px; margin-left:20px;'>" +
                                    (minutes < 61 ? (minutes < 1 ? seconds + " seconds ago" : minutes +
                                        " minutes ago") : hours + " hours ago") + "</small>";
                            }
                            bodyData += "</p></div>";
                            bodyData += "</div></li></a></div>";
                        } else {
